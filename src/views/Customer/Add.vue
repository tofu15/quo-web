...
import type {CusPost} from "@/views/Customer/Interface";
...
const initialTable: CusPost = reactive({
    cname: "",
    address: "",
    email: "",
    zip: "",
...
const isEdited = computed<boolean>(() => {
    return !(table.cname == initialTable.cname
        && table.address == initialTable.address
        && table.email == initialTable.email
        && table.zip == initialTable.zip
        && table.tel == initialTable.tel
        && table.fax == initialTable.fax
        && table.contact == initialTable.contact
        && table.csource == initialTable.csource
        && table.ftime == initialTable.ftime
    )
})
...
function update() {
    status.loading = true
    Post('/api/customer', table).then((rsp) => {
